...
| `list_audiences` / `preview_audience` / `save_audience` / `update_audience` | The audience family — a search becomes a durable, rolling audience by saving the same filters. **Build with named `parts`** (each with a marketer-readable name + description you write — the UI hover card) so the operator can understand and toggle each rule independently; `update_audience` `toggle_parts` parks a part without deleting it. See "An audience is a set of rules" in the recipes. |
| `create_custom_filter` / `list_custom_filters`                              | Named, DESCRIBED, SQL-backed rules for slices the grammar cannot say (grammar first — this is the escape hatch). Narrow-only by construction; description required; reference from any rule via `filters.customFilterIds`. Check the list before writing a new one.                                                                                                                                    |
